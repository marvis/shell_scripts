...
do
	...
	to_echo_script.sh lesson$i.c makefile > nehe_0${i}_macos.sh
	chmod +x nehe_0${i}_macos.sh
done
...
do
	...
	to_echo_script.sh lesson$i.c makefile > nehe_0${i}_linux.sh
	chmod +x nehe_0${i}_linux.sh
done
